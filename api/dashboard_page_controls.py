      ...
      <div class='filter-grid'>
        ...
        <fieldset class='kline-settings'>
          ...
          <div class='field-stack'>
            ...
            <div class='form-field sort-combo-field'><span>圖塊排序</span><div class='sort-combo' role='group' aria-label='圖塊排序'><select name='card_sort' aria-label='圖塊排序屬性'><option value='symbol' {'selected' if card_sort=='symbol' else ''}>個股代號</option><option value='signal_score' {'selected' if card_sort=='signal_score' else ''}>形勢分數</option><option value='close' {'selected' if card_sort=='close' else ''}>成交價</option><option value='volume' {'selected' if card_sort=='volume' else ''}>成交量</option><option value='change_pct' {'selected' if card_sort=='change_pct' else ''}>漲跌幅度</option><option value='target_ratio' {'selected' if card_sort=='target_ratio' else ''}>目標價/現價</option></select><select name='card_sort_direction' class='sort-direction-select' aria-label='圖塊排序方向'><option value='desc' {'selected' if card_sort_direction!='asc' else ''}>Z→A ↓</option><option value='asc' {'selected' if card_sort_direction=='asc' else ''}>A→Z ↑</option></select></div></div>
            <div class='form-field kline-display-field'><span>K線顯示</span><div class='kline-display-combo' role='group' aria-label='K線顯示設定'><select name='show_price' aria-label='顯示價K線'><option value='1' {'selected' if show_price else ''}>價K 開啟</option><option value='0' {'selected' if not show_price else ''}>價K 關閉</option></select><select name='show_volume' aria-label='顯示量K線'><option value='1' {'selected' if show_volume else ''}>量K 開啟</option><option value='0' {'selected' if not show_volume else ''}>量K 關閉</option></select></div></div>
