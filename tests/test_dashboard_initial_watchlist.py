...
class DashboardInitialWatchlistTests(unittest.TestCase):
    def test_default_watchlist_uses_file_watchlist_not_entire_llm_metadata(self) -> None:
        ...
        self.assertIn("table th:nth-child(15), table td:nth-child(15){width:calc(190px + 3em);min-width:calc(190px + 3em);max-width:calc(190px + 3em)}", response)
    ...
    def test_dashboard_renders_loading_progress_for_slow_category_requests(self) -> None:
        ...
        self.assertIn("fieldName === 'card_sort' || fieldName === 'card_sort_direction'", response)
        self.assertIn("function sortedDashboardItems", response)
        self.assertIn('"sort_metrics"', response)
        self.assertIn("未重新下載行情或重算技術分析", response)
